...
public class RNNaverMapInfoWindow extends ClickableRNNaverMapFeature<InfoWindow> {
    ...
    private GenericDraweeHierarchy createDraweeHierarchy() {
        return new GenericDraweeHierarchyBuilder(getResources())
                .setActualImageScaleType(ScalingUtils.ScaleType.FIT_CENTER).setFadeDuration(0).build();
    }

    public void setCoordinate(LatLng latLng) {
        if (animated && this.duration > 0)
            setCoordinateAnimated(latLng, this.duration);
        else
    ...
    public void setTitle(String title) {
        feature.setAdapter(new InfoWindow.DefaultTextAdapter(titleContext) {
